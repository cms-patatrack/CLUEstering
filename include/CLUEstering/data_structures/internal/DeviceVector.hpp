...
namespace clue::internal {
  ...
  public:
    ALPAKA_FN_HOST_ACC constexpr DeviceVectorView(int32_t* data,
                                                  std::size_t* size,
                                                  std::size_t capacity)
    ...
    ALPAKA_FN_ACC constexpr void push_back(const TAcc& acc, int32_t value) {
      auto prev = alpaka::atomicAdd(acc, m_size, static_cast<std::size_t>(1));
      if (prev < m_capacity) {
        m_data[prev] = value;
      } else {
        alpaka::atomicSub(acc, m_size, static_cast<std::size_t>(1));
      }
    }
  };
