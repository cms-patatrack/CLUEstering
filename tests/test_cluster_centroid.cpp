...
TEST_CASE("Test computation of cluster centroid") {
  ...
  SUBCASE("Check weighted centroid of cluster 0") {
    auto centroid = clue::weighted_cluster_centroid(h_points, 0);
    std::ranges::for_each(centroid, [](auto coord) -> void {
      CHECK(std::isfinite(coord));
      CHECK(!std::isnan(coord));
    });
...
TEST_CASE("Test computation of all weighted cluster centroids") {
  const auto device = clue::get_device(0u);
  clue::Queue queue(device);
  ...
  algo.make_clusters(queue, h_points, d_points);
  auto centroids = clue::weighted_cluster_centroids(h_points);
  auto clusters = clue::get_clusters(h_points);
  CHECK(centroids.size() == clusters.size());
}
