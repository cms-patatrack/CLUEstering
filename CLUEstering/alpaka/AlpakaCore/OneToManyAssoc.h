...
namespace cms::alpakatools {
  ...
  public:
    ...
    constexpr const index_type* begin() const { return content.data(); }
    constexpr index_type* begin() { return content.data(); }
    constexpr const index_type* end() const { return begin() + size(); }
    constexpr index_type* end() { return begin() + size(); }

    constexpr const index_type* begin(uint32_t b) const {
      return content.data() + off[b];
    }
    constexpr index_type* begin(uint32_t b) { return content.data() + off[b]; }

    constexpr const index_type* end(uint32_t b) const {
      return content.data() + off[b + 1];
    }
    constexpr index_type* end(uint32_t b) { return content.data() + off[b + 1]; }
